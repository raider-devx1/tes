<x-app-layout title="Edit Informasi">
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <form method="POST" action="{{ route('admin.informasi.update', $informasi) }}" class="bg-white rounded-xl border border-blue-100 p-6 space-y-5">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="kategori" required class="w-full rounded-lg border-gray-200 focus:border-[#2563EB] focus:ring-[#2563EB]">
                        @foreach($kategoriLabels as $key => $label)
                            <option value="{{ $key }}" {{ old('kategori', $informasi->kategori) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
