<x-app-layout title="Monitoring Catatan">
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <form method="GET" class="bg-white rounded-xl border border-blue-100 p-4 flex flex-wrap gap-3 items-end">
            ...
            <a href="<?= e(route('admin.monitoring.catatan')) ?>" class="px-4 py-2 rounded-lg text-gray-500 text-sm hover:bg-gray-50">Reset</a>
        </form>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($catatan as $c)
                            <tr class="hover:bg-blue-50/40">
                                <td class="px-4 py-3 font-medium text-gray-800"><?= e($c->user->name ?? '-') ?></td>
                                <td class="px-4 py-3"><?= e($c->nama_pekerjaan) ?></td>
                                <td class="px-4 py-3 text-gray-600"><?= e(\Illuminate\Support\Str::limit($c->perencanaan_kegiatan, 60)) ?></td>
                                <td class="px-4 py-3 text-gray-600"><?= e(\Illuminate\Support\Str::limit($c->pelaksanaan_kegiatan, 60)) ?></td>
                        ...
                        @empty
                            <tr><td colspan="5" class="px-4 py-8 text-center text-gray-400">Tidak ada data catatan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
